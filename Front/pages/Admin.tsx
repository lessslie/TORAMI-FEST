...
import {
  ...
  getCosplayRegistrations, updateCosplayStatus, deleteCosplayRegistration,
  getCosplayGuests, getCosplayGuestById, updateCosplayGuestStatus, deleteCosplayGuest,
...
import { Plus, Edit, Trash2, Check, X, Ghost, Image, Gift, Calendar, Store, DollarSign, Upload, ExternalLink, ZoomIn, Save, AlertTriangle, RefreshCw, Link as LinkIcon, Film, Trophy, Eye, Mic2, Phone, Users, Sparkles, Star, Clock, Instagram, Globe, Mic, Music, Download, ToggleLeft, ToggleRight } from 'lucide-react';
                        ...
                        {isRejectingStand ? (
                            <div className="animate-in fade-in slide-in-from-bottom-2 bg-red-50 p-4 border border-red-200 rounded">
                                <label className="flex text-sm font-bold mb-2 uppercase text-red-600 items-center gap-2">
                                    <AlertTriangle size={16} /> Motivo del Rechazo (Opcional)
                                </label>
                                <textarea
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 p-4 backdrop-blur-md">
          <div className="bg-white border-4 border-red-600 shadow-manga w-full max-w-md animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              ...
              <p className="text-gray-700 mb-2">Esta acción <span className="font-bold text-red-600">no se puede deshacer</span>.</p>
              <p className="text-sm text-gray-600 mb-4">Se eliminará el registro permanentemente.</p>
                  ...
                  <div>
                      ...
                      <div className="border-2 border-dashed border-gray-400 bg-gray-50 p-4 text-center cursor-pointer hover:bg-gray-100 transition-colors relative">
                          <input
                            ...
                            disabled={isUploadingOfficialPhoto}
                            onChange={async (e: React.ChangeEvent<HTMLInputElement>) => {
                                if (e.target.files && e.target.files[0]) {
                                    ...
                                    try {
                                        ...
                                        setOfficialUploadData(prev => ({ ...prev, url: result.secure_url }));
                                    } catch (error) {
                                        console.error('Error uploading to Cloudinary:', error);
                                        alert('❌ Error subiendo la imagen. Intenta nuevamente.');
                                    } finally {
                          ...
                          />
                          {isUploadingOfficialPhoto ? (
                              <div className="text-gray-500 py-8">
                                  <RefreshCw className="mx-auto mb-2 animate-spin" size={32} />
                                  <span className="text-sm font-bold uppercase">Subiendo imagen a Cloudinary...</span>
                              </div>
